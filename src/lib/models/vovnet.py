...
class _OSA_stage(nn.Sequential):
    def __init__(self,
        ...
        #         nn.MaxPool2d(kernel_size=3, stride=2, ceil_mode=True, return_indices=True))
...
class VoVNet(nn.Module):
    def __init__(self, 
        ...
        for head in self.heads:
            ...
            if self.head_conv > 0:
              fc = nn.Sequential(
                  nn.Conv2d(config_concat_ch[-1], self.head_conv,
              ...
              fc = nn.Conv2d(config_concat_ch[-1], classes, 
                  kernel_size=1, stride=1, 
                  padding=1 // 2, bias=True)
              if 'hm' in head:
    ...
    def forward(self, x):
        ...
        for head in self.heads:
            ...
            print("vov-{}: ".format(head), outputs[head].size())
...
def vov_net(num_layers, heads, head_conv):
    ...
    layer_per_block, pretrained, progress = arch_setting[num_layers]
    print('VOVOVOVOVOOVOVO_{}'.format(heads))
    cfg = {
        'heads': heads,
        'head_conv': 256,
    }
